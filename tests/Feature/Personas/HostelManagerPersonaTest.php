...
class HostelManagerPersonaTest extends TestCase
{
    ...
    {
        $manager = User::create([
            'name' => 'Manager Dup Check',
            'email' => 'mgr_dup@example.com',
            'password' => Hash::make('password123'),
            'phone' => '08011122290',
            'role' => 'hostel_manager',
            'email_verified_at' => now(),
        ...
        $payload = [
            'number' => '101', // Attempting to rename room 102 to 101 which already exists
            'room_type' => 'single_room',
            'capacity' => 2,
            'price_per_semester' => 180.00,
            'gender' => 'any',
            'status' => 'available',
        ];

        $this->actingAs($manager)
            ->put(route('hostel-manager.rooms.update', ['room' => $roomTwo->uuid]), $payload)
            ->assertRedirect()
