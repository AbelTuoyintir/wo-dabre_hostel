...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <h5 class="text-sm font-semibold text-gray-800">Edit Hostel</h5>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="p-6">
            <form action="{{ route('agent.hostels.update', $hostel->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="number" step="any"
                                       ...
                                       id="latitude" name="latitude" value="{{ old('latitude', $hostel->latitude) }}">
                                @error('latitude')
                                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                ...
                                <input type="number" step="any"
                                       ...
                                       id="longitude" name="longitude" value="{{ old('longitude', $hostel->longitude) }}">
                                @error('longitude')
                                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <textarea class="w-full px-3 py-1.5 text-xs border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-500 @enderror"
                                      id="description" name="description" rows="6" required>{{ old('description', $hostel->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" min="0"
                                   ...
                                   id="agent_fee" name="agent_fee" value="{{ old('agent_fee', $hostel->agent_fee ?? 100) }}">
                            ...
                            <div class="relative group cursor-pointer border-2 border-dashed border-gray-300 rounded-xl p-6 bg-gray-50 hover:bg-indigo-50/30 hover:border-indigo-500 transition-all duration-300 text-center" id="featured-dropzone">
                                ...
                                <div class="space-y-2" id="featured-prompt">
                                    ...
                                    <div class="text-xs text-gray-600">
                                        <span class="font-semibold text-indigo-600 hover:text-indigo-500">Click to upload new cover</span> or drag and drop
                                    </div>
                        ...
                        <div class="space-y-3">
                                        ...
                                        @foreach($hostel->galleryImages as $image)
                                            <div class="relative group border rounded-lg overflow-hidden shadow-sm aspect-video flex items-center justify-center bg-black" data-gallery-id="{{ $image->id }}">
                                                ...
                                                <button type="button" onclick="confirmDeleteImage({{ $image->id }})"
                                                    class="absolute top-1.5 right-1.5 bg-red-500 hover:bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition z-20">
                                                    &times;
                                                </button>
                                            </div>
                                        @endforeach
                            ...
                            <div class="relative group cursor-pointer border-2 border-dashed border-gray-300 rounded-xl p-6 bg-gray-50 hover:bg-indigo-50/30 hover:border-indigo-500 transition-all duration-300 text-center" id="gallery-dropzone">
                                ...
                                <div class="space-y-2" id="gallery-prompt">
                                    ...
                                    <div class="text-xs text-gray-600">
                                        <span class="font-semibold text-pink-600 hover:text-pink-500">Upload new gallery files</span> or drag and drop
                                    </div>
...
<!-- JavaScript for Preview and Deletion -->
<script>
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if (imgBox) {
                    ...
                    badge.innerHTML = '<span>Removed</span>';
                    imgBox.appendChild(badge);
                }

                Swal.fire({
                    title: 'Marked for removal',
                    text: "Changes will be applied when you save the hostel.",
                    icon: 'success',
                    timer: 1500,
    ...
    // Play videos on hover (both current and new ones)
    document.querySelectorAll('[data-gallery-id] video').forEach(video => {
    ...
    document.querySelector('button[type="reset"]').addEventListener('click', function(e) {
        // Delay resetting elements to allow default form reset to clear input fields
        setTimeout(() => {
            removeFeaturedImage();
            ...
            // Restore current gallery media opacities
            document.querySelectorAll('[data-gallery-id]').forEach(imgBox => {
                imgBox.style.opacity = '1';
                imgBox.style.pointerEvents = 'auto';
                const badge = imgBox.querySelector('.bg-red-500\\/20');
                if (badge) badge.remove();
            });

            // Clear removed_images input values
            document.getElementById('removed-images-container').innerHTML = '';
            removedImages = [];
        }, 50);
    });
