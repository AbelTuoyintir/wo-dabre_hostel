...
@section('content')
    ...
    <!-- MAIN LISTINGS GRID - More Compact -->
    <section class="container mx-auto px-4 md:px-6 py-4 md:py-6">
        <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-4 gap-y-6">
            @if(isset($transformedHostels) && count($transformedHostels) > 0)
                @foreach($transformedHostels as $hostel)
                    ...
                    <div class="group relative flex flex-col gap-2">
                        ...
                        <button class="wishlist-btn absolute top-2 right-2 bg-white/90 text-slate-600 hover:text-rose-500 shadow-sm border border-slate-200/50 text-xs z-10 hover:scale-110 transition-transform focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 focus-visible:ring-offset-2 rounded-full w-7 h-7 flex items-center justify-center"
                        ...
                        <label class="absolute top-2 left-2 z-10 cursor-pointer focus-within:ring-2 focus-within:ring-rose-500 focus-within:ring-offset-2 rounded-full outline-none" title="Compare {{ $hostel['name'] }}" onclick="event.stopPropagation()">
                            ...
                            <div class="bg-white/90 p-1.5 rounded-full shadow-sm border border-slate-200 hover:bg-white transition-colors flex items-center justify-center w-6 h-6 group-has-[:checked]:bg-rose-500 group-has-[:checked]:border-rose-500">
                                <i class="fas fa-plus text-[8px] text-slate-600 group-has-[:checked]:hidden" aria-hidden="true"></i>
                                <i class="fas fa-check text-[8px] text-white hidden group-has-[:checked]:inline-block" aria-hidden="true"></i>
                            </div>
                        </label>
                    </div>
                @endforeach
            @else
                <div class="col-span-full text-center py-16 bg-slate-50 rounded-xl border-2 border-dashed border-slate-200">
                    ...
                    <a href="{{ route('hostels.index') }}" class="mt-4 inline-block bg-slate-800 text-white px-5 py-1.5 rounded-lg text-sm font-medium hover:bg-slate-900">
                        Clear all filters
                    </a>
                </div>
            @endif
...
<!-- FLOATING COMPARISON BAR - More Compact -->
    ...
    <style>
        ...
        /* More compact card grid for mobile */
