...
$hostel->images()->create([
    ...
    public function createRoom(Request $request, ?Hostel $hostel = null)
    {
        ...
        } elseif ($request->query('hostel_id') || $request->query('hostel')) {
            $hostelId = $request->query('hostel_id') ?? $request->query('hostel');
            $selectedHostel = $this->getAgentHostelQuery($agent)
                ->where(function ($query) use ($hostelId) {
                    $query->where('id', $hostelId)->orWhere('uuid', $hostelId);
                })
                ->first();
    ...
    public function storeRoom(Request $request)
    {
        ...
        $hostel = $this->getAgentHostelQuery($agent)
            ->where(function ($query) use ($request) {
                $query->where('id', $request->hostel_id)->orWhere('uuid', $request->hostel_id);
            })
            ->first();

        if (!$hostel) {
            abort(403, 'Unauthorized or invalid hostel.');
        }
